...
@endsection
@section('content')
<div class="card jenis_kelamin_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
                            ...
                            @if(Gate::allows('jenis_kelamin_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("manage.jenis_kelamin.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->jenis_kelamin}}</td>
                            <td class="w-5 no-sort"><a href="{{route("manage.jenis_kelamin.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['jenis_kelamin_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['jenis_kelamin_allow']))
                        <a href="{{route('manage.jenis_kelamin.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("manage.jenis_kelamin.delete")}}">
            @method('DELETE')
            @csrf
