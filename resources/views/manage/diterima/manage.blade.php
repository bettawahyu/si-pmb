...
@endsection
@section('backBtn')
<a href="{{ route("manage.diterima.index") }}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card formPage diterima_manage admikoForm">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelectSort">
                        ...
                        <div class="col-md-10" style="position: relative">
                            <select name="siswa_yang_diterima[]" data-placeholder="{{trans('admiko.select_from_list')}}" multiple="multiple" id="siswa_yang_diterima" required="true" style="width: 100%" data-allow-clear="true">
                            ...
                            @foreach($pendaftar_all as $id => $value)
                                ...
                                @elseIf(isset($data) && $data->siswa_yang_diterima_many->contains($id))
                                    @php $selected = "selected"; @endphp
                                    @php $orderId = $data->siswa_yang_diterima_many->firstWhere('id', $id)->pivot->admiko_order; @endphp
                                @endIf
                                <option value="{{ $id }}" {{$selected}} data-order="{{$orderId}}">{{ $value }}</option>
                            @endforeach
                            </select>
                            <div class="invalid-feedback @if ($errors->has('siswa_yang_diterima')) d-block @endif">{{trans('admiko.required_text')}}</div>
                ...
                <div class=" col-lg-6">
                    <div class="form-group row">
                        ...
                        <div class="col-md-10 col-lg-8">
                            ...
                            <div class="invalid-feedback @if ($errors->has('tanggal_daftar_ulang')) d-block @endif">{{trans('admiko.required_text')}}</div>
                ...
                <div class=" col-lg-6">
                    <div class="form-group row">
                        ...
                        <div class="col-md-10 col-lg-8">
                            ...
                            <div class="invalid-feedback @if ($errors->has('batas_daftar_ulang')) d-block @endif">{{trans('admiko.required_text')}}</div>
        ...
        <div class="card-footer form-actions" id="form-group-buttons">
            <div class="row">
                ...
                <div class="col-sm-10">
                    ...
                    <a href="{{ route("manage.diterima.index") }}" class="btn btn-secondary float-end" role="button">{{trans('admiko.table_cancel')}}</a>
                </div>
            </div>
        </div>
    </form>
