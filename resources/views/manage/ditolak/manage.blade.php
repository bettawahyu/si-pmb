...
@endsection
@section('backBtn')
<a href="{{ route("manage.ditolak.index") }}"><i class="fas fa-angle-left"></i> {{trans('dokre.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card formPage ditolak_manage dokreForm">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelect">
                        ...
                        <div class="col-md-10" style="position: relative">
                            <select name="siswa_yang_ditolak[]" data-placeholder="{{trans('dokre.select_from_list')}}" multiple="multiple" id="siswa_yang_ditolak" required="true" style="width: 100%" data-allow-clear="true">
                            ...
                            @foreach($pendaftar_all as $id => $value)
                                ...
                                @elseIf(isset($data) && $data->siswa_yang_ditolak_many->contains($id))
                                    @php $selected = "selected"; @endphp
                                    @php $orderId = $data->siswa_yang_ditolak_many->firstWhere('id', $id)->pivot->dokre_order; @endphp
                                @endIf
                                <option value="{{ $id }}" {{$selected}} data-order="{{$orderId}}">{{ $value }}</option>
                            @endforeach
                            </select>
                            <div class="invalid-feedback @if ($errors->has('siswa_yang_ditolak')) d-block @endif">{{trans('dokre.required_text')}}</div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <select class="form-select" id="status_penolakan" name="status_penolakan" >
                                <option value="">{{trans("dokre.select")}}</option>
                            ...
                            <div class="invalid-feedback @if ($errors->has('status_penolakan')) d-block @endif">{{trans('dokre.required_text')}}</div>
        ...
        <div class="card-footer form-actions" id="form-group-buttons">
            <div class="row">
                ...
                <div class="col-sm-10">
                    ...
                    <a href="{{ route("manage.ditolak.index") }}" class="btn btn-secondary float-end" role="button">{{trans('dokre.table_cancel')}}</a>
                </div>
            </div>
        </div>
    </form>
