        ...
        var oTable = $(".tableBox table").DataTable({
            ...
            initComplete: (settings) => {
                $(settings.nTableWrapper)
                    ...
                    .appendTo(".paginationBox");
                $(settings.nTableWrapper)
                    .find(".dataTables_length")
                    .appendTo(".lengthTable")
                    .find("select")
                    .removeClass("custom-select-sm");
                $(settings.nTableWrapper)
                    .find(".dataTables_info")
                    .appendTo(".paginationInfo");
            },
...
$(".tableLayout").on("click", ".dokre_deleteConfirm", function (event) {
    event.preventDefault();
    $(".dataDelete").html("");
        ...
        if (typeof $(this).data("type") !== "undefined" && this.files[0]) {
            ...
            if ($(this).data("type").split(",").indexOf(extension) < 0) {
                ...
                errorText =
                    errorText +
                    " " +
                    $(this).siblings(".invalid-feedback").data("type");
            }
        }
        ...
        if (typeof $(this).data("size") !== "undefined" && this.files[0]) {
            ...
            if (fileSize > $(this).data("size")) {
                ...
                errorText =
                    errorText +
                    " " +
                    $(this).siblings(".invalid-feedback").data("size");
            }
        }
...
function minMaxNumbersCheck(element) {
    ...
    if (
        typeof element.data("min") !== "undefined" ||
        typeof element.data("max") !== "undefined"
    ) {
...
function updateOrder() {
    ...
    $.post(
        reorderUrl,
        { action: "dokre_sort", sortData: data_array, _token: csrf_token },
        function (data) {
            if (data != "done") {
                console.log(data);
            }
...
function richTextEditor() {
    tinymce.init({
        ...
        toolbar:
            "bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link | removeformat | undo redo | code",
    ...
    tinymce.init({
        ...
        toolbar:
            "bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | forecolor backcolor | link codesample | fontselect fontsizeselect formatselect | removeformat | undo redo | code",
        toolbar_mode: "sliding",
        statusbar: false,
        height: 250,
    });
...
function startGoogleMaps() {
    $(".dokre_google_map").each(function () {
        googleMap($(this));
    });
}
...
function googleMap(mapBox) {
    ...
    var latlng = "";
    var map_lat = mapBox.find(".dokre_map_lat").val();
    var map_lng = mapBox.find(".dokre_map_lng").val();
    ...
    google.maps.event.addListener(marker, "drag", function (event) {
        mapBox.find(".dokre_map_lat").val(event.latLng.lat());
        mapBox.find(".dokre_map_lng").val(event.latLng.lng());
    });
}

function startBingMaps() {
    $(".dokre_bing_map").each(function () {
        bingMap($(this));
    });
}
...
function bingMap(mapBox) {
    ...
    Events.addHandler(pin, "dragend", function () {
        mapBox.find(".dokre_map_lat").val(pin.getLocation().latitude);
        mapBox.find(".dokre_map_lng").val(pin.getLocation().longitude);
    });
}
...
function dokreGlobalSearchStart() {
